import { createRoute, z } from '@hono/zod-openapi';
import { createErrorResponse } from '~/utils/error-response-factory';

export const bookSchema = z
  .object({
    id: z.string(),
    title: z.string(),
    author: z.string(),
    yearpublish: z.coerce.number().nullable(),
    avaliable: z.coerce.boolean(),
    created: z.coerce.string().datetime(),
    updated: z.coerce.string().datetime(),
  })
  .openapi('Book');

export const librarySchema = z
  .object({
    totalBooks: z.number(),
    totalAvailable: z.number(),
  })
  .openapi('LibraryStatus');

export const getLibraryStatusRoute = createRoute({
  operationId: 'getLibraryStatus',
  tags: ['books'],
  method: 'get',
  path: '/api/library/status',
  responses: {
    200: {
      description: 'Mendapatkan total buku dan total available',
      content: { 'application/json': { schema: librarySchema } },
    },
    500: createErrorResponse('GENERIC', 'Internal Server Error'),
  },
});

export const getBookRoute = createRoute({
  operationId: 'getBooks',
  tags: ['books'],
  method: 'get',
  path: '/api/books',
  request: {
    query: z.object({
      isAvailable: z
        .string()
        .optional()
        .transform((val) =>
          val === undefined ? undefined : val.toLowerCase() === 'true',
        )
        .openapi({
          type: 'boolean',
          description: 'Filter berdasarkan ketersediaan (true/false)',
        }),
    }),
  },
  responses: {
    200: {
      description: 'Mendapatkan semua buku',
      content: { 'application/json': { schema: z.array(bookSchema) } },
    },
    500: createErrorResponse('GENERIC', 'Internal Server Error'),
  },
});

export const getById = createRoute({
  operationId: 'getById',
  tags: ['books'],
  method: 'get',
  path: '/api/books/{id}',
  request: {
    params: z.object({
      id: z.string().openapi({ description: 'ID Buku' }),
    }),
  },
  responses: {
    200: {
      description: 'Mendapatkan buku dengan id tertentu',
      content: { 'application/json': { schema: bookSchema } },
    },
    404: createErrorResponse('GENERIC', 'Buku tidak ditemukan'),
    500: createErrorResponse('GENERIC', 'Internal Server Error'),
  },
});

export const newBook = createRoute({
  operationId: 'newBook',
  tags: ['books'],
  method: 'post',
  path: '/api/books',
  request: {
    body: {
      content: {
        'application/json': {
          schema: z.object({
            title: z.string(),
            author: z.string(),
            publishedYear: z.number().optional().nullable(),
          }),
        },
      },
    },
  },
  responses: {
    201: {
      description: 'Berhasil memasukkan buku baru',
      content: { 'application/json': { schema: bookSchema } },
    },
    500: createErrorResponse('GENERIC', 'Internal Server Error'),
  },
});

export const editBook = createRoute({
  operationId: 'editBook',
  tags: ['books'],
  method: 'patch',
  path: '/api/books/{id}',
  request: {
    params: z.object({
      id: z.string().openapi({ description: 'ID Buku' }),
    }),
    body: {
      content: {
        'application/json': {
          schema: z.object({
            title: z.string().optional().nullable(),
            author: z.string().optional().nullable(),
            isAvailable: z.boolean().optional().nullable(),
            publishedYear: z.number().optional().nullable(),
          }),
        },
      },
    },
  },
  responses: {
    200: {
      description: 'Berhasil mengedit buku',
      content: { 'application/json': { schema: bookSchema } },
    },
    404: createErrorResponse('GENERIC', 'Buku tidak ditemukan'),
    500: createErrorResponse('GENERIC', 'Internal Server Error'),
  },
});

export const deleteBook = createRoute({
  operationId: 'deleteBook',
  tags: ['books'],
  method: 'delete',
  path: '/api/books/{id}',
  request: {
    params: z.object({
      id: z.string().openapi({ description: 'ID Buku' }),
    }),
  },
  responses: {
    200: {
      description: 'Menghapus buku sukses',
      content: {
        'application/json': {
          schema: z.object({ message: z.string() }),
        },
      },
    },
    404: createErrorResponse('GENERIC', 'Buku tidak ditemukan'),
    500: createErrorResponse('GENERIC', 'Internal Server Error'),
  },
});
